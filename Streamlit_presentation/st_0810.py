...
GBC_2_vectorizer = pickle.load(open("vectoriser_GBC_2","rb"))
Bernoulli_vectorizer = pickle.load(open("vectoriser_ber","rb"))

# load pre-trained model
trained = pickle.load(open(r"trained.pickle","rb"))

# load Model For Prediction
GBC_2_model = pickle.load(open("GBC_2.pickle","rb"))
Bernoulli_model = pickle.load(open("ber.pickle","rb"))
SVM_model = pickle.load(open("SVM.pickle","rb"))


# on charge un df avec pandas
#df_origin = pd.read_csv('reviews_trust.csv',index_col = 0)
df_origin = pd.read_csv('reviews_trust.csv')
df = pd.read_csv('reviews_trust_fr_VF.csv',index_col = 0) 

data = pd.read_csv('review_trust_fr_lemmantiser_word+2_VF.csv', sep=',', index_col = 0)

## 🛑🛑@ pas ce document chez mon PC🛑🛑
df_errors_GBC = pd.read_csv('sav_verif_erreurs_model2.csv',index_col = 0)
df_errors_wiki_SVM = pd.read_csv('sav_verif_erreurs_model4.csv',index_col = 0)
...
# Mot particulier
		...
		if st.checkbox("📎 Mots particulers = Mots importants à traiter"):
			...
			st.image('ras-rad_bonne_imagex3.png')
...
def GBC_2(corpus):
	# charger le vecteur
	# 🌞🌞🌞@fred: changer le file path 🌞🌞🌞
	vectorizer = pickle.load(open('vectoriser_GBC_2','rb'))
	# charger le model 
	# 🌞🌞🌞@fred: changer le file path 🌞🌞🌞
	GBC_2 = pickle.load(open('GBC_2.pickle','rb'))
...
if page ==pages[0]:  # sur la page 0 Introduction
	# affichage
	insert_head('https://datascientest.fr/train/assets/logo_datascientest.png','logo SatisPy Project.png')
	...
	st.write("On nous a remis un dataset 'reviews_trust.csv' comportant 19.863 lignes et 11 colonnes qui correspond aux commentaires clients et notation de 1 à 5 étoiles sur leurs achats de produit sur 2 sites marchands 'ShowRoom' et 'VeePee'. Ces commentaires proviennent de 2 sources récoltant les avis, 'TrustedShop' et 'TrustPilot' et voici un extrait du dataset :")	
	# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
	st.image("1 extrait dataset.jpg")
	

	st.write("")
	st.write("Une petite fonction créé pour l'occasion pour permet d'obtenir une description globale des statisques sur chaque variable.")
	# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
	st.image("2 describ total.jpg")
	#st.write('add image')


	st.write("")
	st.write("### Déroulement de nos réfléxions, tests et découvertes.")
	st.write("#### 2 parties essentielles avant de commencer à modifier nos données.")
	st.write("- Analyse visuelle de nos données 👉 DataViz (ici en page 2)")
	st.write("- Analyse du Verbatim des commentaires (connaître les liens et contexte des mots)")
	st.write("")
	st.write("#### Objectifs qui ont évolués au fur et à mesure")
	st.write(":one: Travailler uniquement sur les commentaires Francais pour prédire un sentiment Positif ou Négatif.")
	st.write(":two: Trouver les ressources par nos modules de cours et recherches @ afin de ne pas 'réinventer la roue' (en moins bien)")
	st.write(":three: Apprendre de nos échecs comme de nos réussites 👉 Tableaux des erreurs (ici en page 4)")
	st.write(":four: Découvrir de nouvelles approches ")
	st.write(":five: Transmettre à tous notre méthodologie de manière ludique 👉 tests à la demande (ici en page 4)")
...
#------------------------------------------------------------------------------------------
	...
	with col5:    
		if st.button('Aide @ Gradient Boosting'):
			...
			webbrowser.open_new_tab(url3)
# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
	st.image ("gradient boosting classifier.png")
	#st.write('add image')
	...
	with col5:    
		if st.button('Aide vidéo Tuto wikipedia2vec'):        
			...
			webbrowser.open_new_tab(url6)
# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
	st.image("schema_principe_wikipedia2vec.jpg")
	...
	st.image("support-vector-machines-2.png")
	#st.write('add img')
...
# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
		st.image("confusion_classifreport_model2.jpg") 
		#st.write('add image')
...
# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
		st.image("confusion_classifreport_model4.jpg")
		#st.write('add img')
...
# 🛑🛑ajouter les image dans le dossier et décommenté🛑🛑
		st.image("ANN-layers_fit_confusion_classifreport.jpg")
		#st.write('add img')
